export default {
    getMovieListData(message) {
        const server = require("../js/config");
        console.log(message)
        const url = `${server.default.HTTP}` + `${server.default.SERVER_PATH}` + `:` + `${server.default.PORT}` + `/getMovieListData`+`?message=${message}`;
    ...
    getMovieDetailData(id) {
        const server = require("../js/config");
        const url = `${server.default.HTTP}` + `${server.default.SERVER_PATH}` + `:` + `${server.default.PORT}` + "/getMovieDetailData" + `?id=${id}`;
        return new Promise(function (resolve, reject) {
            fetch(url).then((response) => {
                if (response.ok) {
                    return response.json()
    ...
    getSearchMovieData(message) {
        const server = require("../js/config");
        console.log(message)
        const url = `${server.default.HTTP}` + `${server.default.SERVER_PATH}` + `:` + `${server.default.PORT}` + `/getSearchMovieData`+`?message=${message}`;
        return new Promise(function (resolve, reject) {
            fetch(url).then((response) => {
                if (response.ok) {
                    return response.json()
